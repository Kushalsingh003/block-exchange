...
export default function Login() {
  ...
  const phoneNumberPasswordInputRef = useRef();
  ...
  async function emaillogin(data) {
    try {
      let res = await axios.post("/api/login/loginEmailApi", data);
      const response = res.data;
      console.log(response, "to get the response from the api for email");
      const fa = response.data.data.fa;
      // const cp = response.data.data.cp;
      console.log(fa, "to get fa status");
      // console.log(cp, "to get cp status");
      ...
      setLoadingRef(true);
      // if (cp != ture) {
      //   toast.error("Please SignUp & Create your porfile firs!");
      //   setTimeout(() => {
      //     router.push("/signup/Singup");
      //   }, timeout);
      //   router.push();
      // }
      if (fa != true) {
        router.push("/login/otpLogin");
        return;
      } else {
        router.push("/login/otpFaLogin");
      }
    } 
    catch (err){
      console.log(err, "to get the status of error")
      const errorStatus1 = err?.response.data.Error;
       if(errorStatus1.status == 401) {
        toast.error("User doesn't Exist Please SignIn First")
       }
       else {
  ...
  async function phonelogin(data) {
    try {
      let res = await axios.post("/api/login/loginPhnApi", data);
      const response = res.data;
      console.log(response, "to get the response from the api for phone");
      toast.success("User Logged In Successfully");
      toast.success("OTP sent to your Phone number");
      setTimeout(() => {
        router.push("/login/otpPhnLogin");
      }, 1000);
    } catch (err) {
      console.log(err);
      toast.error("Invalid Phone Number and Data");
    }
  }

  async function phoneFormSubmitHandler(e) {
    e.preventDefault();

    const phone = phoneNumberInputRef.current.value;
    const password = phoneNumberPasswordInputRef.current.value;
        ...
        <div className="container">
          <div className="row justify-content-center">
            ...
            <form className="input-sec ">
              <h3 className="heading-text">Block Exchange Account Login</h3>
            ...
            {session ?
             ...
             <button onClick={()=> signOut()}>signout</button> 
              ...
              <button onClick={()=> signIn()}>login</button>
                ...
                {/* Phone Number Form starts from Here */}
                <div
                ...
                >
                  <div className="input-item">
                    <h6 className="item-text">Phone Number</h6>
                  ...
                  <div className="input-item">
                    <h6 className="item-text">Password</h6>
                    <input
                      ref={phoneNumberPasswordInputRef}
                      className="textinput"
                      type="password"
                      name="phonePassword"
                      placeholder="Enter your Password"
              ...
              <p className="iptpara-text">
                Don't have an account? <a href="/signup/signup">Signup</a>
              </p>
            </form>
          </div>
        </div>
